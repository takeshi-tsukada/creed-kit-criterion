...
\begin{example}
    ...
    Let \( G \) be the species defined by \( G(2) \defeq 2 \), \( G(\sigma)(x) = \sigma^{-1}(x) \) for every \( x \in F(2) \) and permutation \( \sigma \colon 2 \to 2 \), and \( G(n) \defeq \emptyset \) for every \( n \neq 2 \).
    Then \( \widehat{F}(x) = 2 \) and \( \widehat{G}(y) = y \).
