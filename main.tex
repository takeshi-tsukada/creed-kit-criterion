...
\begin{abstract}
    ...
    Its objects are not mere sets but rather sets whose elements are equipped with groups representing their symmetries, and its morphisms carry actions of these symmetries.
    Profunctors definable by \( \lambda \)-terms are known to possess a certain property in their symmetry actions, a property sufficient for ensuring the functoriality of the matrix representation of profunctors.
    Independently, the symmetry actions of \( \lambda \)-definable profunctors have also been studied through the concepts of creeds and kits.
    
    This paper extends these studies on symmetry actions.
    First, we establish a connection between these different approaches to studying symmetry actions.
    Second, we examine the converse question: how precisely do the functoriality of matrix representations, creeds, or kits characterise the definable profunctors? We show that these notions are sufficient to distinguish correct proof nets from incorrect proof structures for multiplicative linear logic, thereby providing new correctness criteria.
\end{abstract}
