...
\title{Creed-Kit Correctness Criterion \\ on Symmetries in Profunctor Model of Linear Logic}
...
\begin{abstract}
    The (bi)category of profunctors on groupoids is a categorification of the relational model of linear logic.
    Its objects are not mere sets but sets whose elements come equipeed with groups representing their symmetries, and its morphisms carry actions of the symmetries.
    The profunctors definable by \( \lambda \)-terms are known to have a certian property on their symmetry actions, a property that is sufficient for the functoriality of the matrix representation of profunctors.
    Independently, the symmetry actions of \( \lambda \)-definable profunctors have also been studied in terms of creeds and kits.
    This paper deepends these studies on symmetry actions.
    First, we show a connection between these studies on symmetry actions.
    Second, we investigate the converse, namely, how precisely the functoriality of the matrix representations, creeds or kits characterises the definable profunctors.
    We show that these notions are sufficient to distinguish between correct proof-nets from incorrect proof structures for multiplicative linear logic, so these notions provide new correctness criteria.
\end{abstract}

\begin{IEEEkeywords}
    linear logic, profunctor, creed/kit, correctness criterion, matrix representation
\end{IEEEkeywords}

\tk{to do: setup cref names}
...
\newcommand{\InducedProf}[1]{\widetilde{#1}}
