...
A \emph{creed} of a groupoid \( \mathcal{A} \) is a collection \( (\sim_{A,B})_{A,B \in \mathcal{A}} \) of binary relations \( ({\sim_{A,B}}) \subseteq \mathcal{A}(A,B) \times \mathcal{A}(A,B) \) such that
\begin{itemize}
    \item \( \ident_A \sim_{A,A} \ident_A \),
    \item \( f \sim_{A,B} f' \) and \( g \colon B \longrightarrow C \) implies \( (g \circ f) \sim_{A,C} (g \circ f') \),
    \item \( f \sim_{A,B} f' \) and \( g \colon C \longrightarrow A \) implies \( (f \circ g) \sim_{A,C} (f' \circ g) \), and
    \item \( f \sim_{A,A} f' \) implies \( f^n \sim_{A,A} f' \) for every natural number \( n \).
\end{itemize}
A presheaf \( P \in \mathit{psh}(\mathcal{A}) \) induces a creed \( \sim \) defined by
\begin{equation*}
    f \sim_{A,B} g
...
\begin{definition}
    A presheaf \( P \) follows a creed if and only if the induced creed is subsumed by the given creed.
    %
    \qed
\end{definition}

The set of creeds is closed under the union and intersection (i.e.~if \( \sim_i \) is a collection of creeds parameterised by \( i \in I \), then so are \( \bigcap_i ({\sim_i}) \) and \( \bigcup_i ({\sim_i}) \)).
The minimum creed is the equivalence on morphisms, and the maximum creed consists of all pairs.
Creeds are \emph{orthogonal} if their intersection is the identity creed.
